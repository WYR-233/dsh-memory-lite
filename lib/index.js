...
export function apply(ctx, pluginConfig) {
  ...
  const readTopics = () => {
    ...
    try {
      for (const line of readFileSync(indexPath, 'utf8').split(/\r?\n/)) {
        ...
        if (/^主题$/.test(name) && /^说明$/.test(note)) continue // 表头/占位行
        topics.push({ name, note })
      }
    } catch { /* no index yet */ }
    return topics
  }
  ...
  const buildSummary = (force) => {
    ...
    const MAX_TOPICS = 24
    // quality-first: keep every topic WITH its one-line note (11 topics ≈ 370 chars) — the
    // notes are what let the model pick the right search terms on the first try.
    const shown = topics.map((t) => (t.note ? `- ${t.name}：${t.note}` : `- ${t.name}`)).join('\n')
    const more = topics.length > MAX_TOPICS ? `\n（还有 ${topics.length - MAX_TOPICS} 个主题见索引）` : ''
    let text = [
      `记忆库（md 卡片 + Git，跨会话长期记忆）：${memoryRoot}`,
      '',
      '主题与内容（卡片文件名形如 日期_标题.md）：',
      shown + more,
      '',
      '用法：需要回忆已做过的决定、踩过的坑、路径/配置、项目进展时，先调 memory_find 检索卡片（一次给 2~4 个关键词最准），再用 read 打开命中的卡片；不要凭空猜、也不要逐层翻目录。',
      `完整目录：${memoryRoot}\\INDEX.md；规矩法典：RULES.md。`,
    ].join('\n')
